...
func (urp *UrpCrawler)login(stuid string, stuPassword string, captcha string, client *http.Client, activateUrlIdx int) (ok bool, err error) {
	...
	//loginresultHtml, _ := ioutil.ReadAll(loginResp.Body)
	//ioutil.WriteFile("loginResult.html",loginresultHtml,0644)

	doc,er := goquery.NewDocumentFromReader(loginResp.Body)
	if er != nil{
		logger.Error("Cannot parse the body with goquery",zap.String("detail",er.Error()))
		err = er
		return
	}
	fmt.Println(doc.Url)
	framenodes := doc.Find("frame").Nodes
	for _,v := range framenodes{
		fmt.Println(v.Data)
	}
